...
exports.addProduct = async (req, res) => {
  ...
  if(!name) return res.status(400).json({ message: 'You need to give the product a name' });
  if(!description) return res.status(400).json({ message: 'You need to give the product a description' });
  if(!price) return res.status(400).json({ message: 'You need to give the product a price' });
  if(!imageURL) return res.status(400).json({ message: 'You need to give the product a imageURL' });
  ...
  if(!product) return res.status(500).json({ message: 'Something went wrong when creating new product' });
